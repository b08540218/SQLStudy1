...
View 수정하기
    ...
    해당 뷰가 존재하면 수정되고, 만약 존재하지 않으면 새롭게 생성도인다.
...
as
    ...
    from employees where job_id='ST_CLERK';
/*
    뷰 생성시 기준테이블의 컬럼명을 변경해서 출력하고 싶다면 
...
*/
/*
1.select문 작성. null이 있는 사칙연산이 되지 않으므로 nvl()
함수를 통해 0으로 변환 후 연산해야한다.
*/
SELECT
...
별칭으로 컬럼명을 명시해야한다. 그렇지 않으면 뷰 생성이 에러가 발생한다.*/
