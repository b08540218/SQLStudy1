...
날짜형식으로 변환해야 한다. 또한 날짜서식 사이에 한글은 혀용되지 않으므로 더블쿼테이션
으로 감싸줘야 한다.
*/
select
    ceil (months_between(sysdate, to_date('2020년01월01일', 'yyyy"년"mm"월"dd"일"')))
from dual;

/*
last_da() : 해당월의 마지막 날짜를 반환한다.
*/

SELECT last_day('22/04/03') from dual; --4월은 30일까지 있음
SELECT last_day('24/02/01') from dual; --24년을 윤년이므로 29일 출력
