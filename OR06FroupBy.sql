...
dintinc : 동일한 값이 있는 경우 중복된 레코드를 제거한 후 하나의
    레코드만 가져와서 보여준다. 순수한 하나의 레코드 이므로 통계적인
    값을 계산할 수 없다.
*/
...
/*1.부서테이블에서 영업티의 부서번호가 몇번인지 확인한다.
하지만 데이터의 데소문자가 일치하지 않아 결과가 인출되지 않는다.*/
...
*/
--전체 사원중 급여가 가장 적은 지원은 누구일까요??
/*
아래 쿼리문은 에러가 발생된다. 그룹함수는 일반컬럼에 바로 사용할 수 있다.
이와같이 경우에는 뒤에서 학습할 '서브쿼리'를 사용해야한다.*/
SELECT first_name, salary from employees where salary=min(salary);


--전체사원중 가장 낮은 급여는 얼마일까요? 즉 급여의 최소값이 얼마인가요?
SELECT min(salary) from employees; --2100 인출됨
--따라서 2100불 받은 직원을 인출하면 위 질문을 해결할 수 있다.
...
1단계 : 부서가 많은 경우 일일이 부서별로 확인할 수 없으므로 부서를 그룹화한다.
    중복이 제거된 결과로 보이지만 동일한 레코드가 하나의 그룸으로 합쳐진
    결과를 인출한다.
*/
...
추가ㅏ면 에러가 발생한다. 이 경우에는 having절에 조건을 추가해야한다.
