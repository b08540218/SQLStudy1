...
처음으로 실행해보는 질의어(sql문 혹은 Query문이라고함)
...
/* where절에 2개 이상의 조건이 필요하면 and 혹은 or 를 사용할수 있다
 => last_name이 Smith 이면서 급여가 8000인 사원을 인출하시오*/
--컬럼이 문자형이면 싱클쿼테이션으로 감싸준다. 숫자라면 생략한다
...
2개 이상의 컴럼으로 정렬해야 할 경우 콤마로 구분해서 정렬한다.
이경우 첫번째 컬럼으로 정렬된 상태에서 두번째 컬럼이 정렬된다.
*/

/*
시나리오] 사원정보 테이블에서 급여가 낮은 순서대로 높은 순서로 정렬하여 
인출하시오
*/
...
/*
시니라오 부서번호를 내림차순으로 정렬한 후 해당 부서에서 낮은 급여를 받는 직원이
...
: 값이 null이거나 null이 아닌 레코드 반출
컬럼중 null값을 허용하는 경우 값을 입력하지 않으면 null값이 입력되는대
