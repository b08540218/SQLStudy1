...
DECLARE
    ...
    --필요하다면 선언부에서 변수 선언과 동시에 초기화 할수 있다.
    dept_name VARCHAR2(30) := '부서정보없음';-- <-초기화 '부서정보없음', 0
...
/*
시나리오] Basic loop문으로 1부터 10까지의 정수의 합을 
구하는 프로그램을 작성하시오
*/
...
BEGIN
    ...
    dbms_output.put_line('1~10가지의 합은:'|| sumnum);
end;
/
...
BEGIN
    ...
    dbms_output.put_line('1~10가지의 합은:'|| total);
end;
/
...
for문에 reverse를 추가한면 거꾸로 반복할 수 있다.
*/
BEGIN
...
/*
반복문3 : for문
    반복의 횟수를 지정하여 사용할 수 있는 반복문을 반복을 위한
...
커서(Cursor)
    ...
    선언방법]
        ...
        Open 커서명
            ...
            실행되어 결과셋을 얻게된다. Cursor는 결과셋의 첫번째행에 
            위치하게된다.
        Fetch~Info~
            : 결과셋에 하나의 행을 읽어들이는 작업으로 결고겟ㅅ의
            인출(Fetch)후에 Cursor는 다음행으로 이동한다.
        Close 커서명
            : 커서 닫기로 결과셋의 자원을 받납한다. select문장이 모두
            처리된 후 Cursor를 닫아준다
...
DECLARE
    ...
    /* 커서선언 : 부서테이블의 모든 레코드를 조회하는 select문으로
        into절이 없는 형태로 작성. 쿼리의 실생결과  cur1에
        저장된다.*/
    CURSOR cur1 is
        SELECT
