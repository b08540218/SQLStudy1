...
실행해야 한다.
새로운 사용자 계정이 생성된 후 접속 및 쿼리생성 테스트는 CMD(명령프롬프트)
에서 진행한다.
*/
...
/*
2]생성된 사용자 계정에 권한 혹은 역학(Role) 부여
...
/*
    ...
    CMD >>>
        ...
        create table tb_test(
            idx number(5) primary ket); 입력하여 테이블 생성 시도
*/
--테이블 생성 권한 부여
grant create table to test_user1;
/*
    create table 권한 부여 후 테이블 생성 맟 desc 명령으로 생성된
    테이블의 스키마를 확인할 수 있다.
*/
...
/*
    ...
    암호로는 접속할 수 없다. 변경한 암호로 재접속 해야한다.
*/
...
/* 아래 2개의 Role은 오라클에서 기본적으로 제공된다. 접속 및 테이블생성
등의 대부분의 기본작업을 할 수 있는 권한이 포함되어 있다.*/
