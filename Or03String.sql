...
/*
concat(문자열1, 문자열2)
: 문자열 1,2를 서로 연결해서 출력, 문자열 부분을 컬럼명을 사용할 수 있음
...
--3단계 : 이름 사이에 스페이스가 없어 가동성이 떨어지므로 추가
...
*/
SELECT initcap('good'), lower('MORning'), upper('sTR') FROM dual;

/*
시니라오] 사원테이블에서 first_name이 john인 사원을 찾아 출력하시오.
*/
...
SELECT rpad(first_name, 7,'*') FROM employees;
...
-양쪽끝에 문자만 제거되고, 중간의 문자는 제거할 수 없다.
...
SELECT
    ...
    ,ltrim(' 좌측공백제거 ', '좌측')
    /*이 경우 2개의 함수를 중접해서 사용하여 기능을 수행할 수 있다.*/
    ,ltrim(trim(' 좌측공백제거 '), '좌측')
from dual;

/*
substr() : 문자열에서 시작인덱스부터 길이만큼 문자열을 출력
    형식] substr(컬럼, 시작인덱스, 길이)
    
    참고1] 오라클의 인덱스는 1부터 시작이다. 0부터 아님
    참고2] '길이'에 해당하는 인수가 없다면 문자열의 끝까지를 의미한다.
    참고3] 시작인덱스가 음수면 우측끝부터 좌로 인덱스를 적용한다.
*/

SELECT substr('good mornimg john', 8, 4) from dual; --rnin
SELECT substr('good mornimg john', 8) from dual; --r부터 끝까지 인출

--문자열의 길이를 반환
SELECT length('good mornimg john') from dual;
...
*를 출력하면 첫글자를 제외한 남은 길이만큼을 마스킹할 수 있다.
*/
SELECT
    first_name,
...
*/
--특정 문자열을 변경
SELECT replace('good mornimg john', 'mornimg', 'evening') from dual;
--문자열 삭제
SELECT replace('good mornimg john', 'mornimg', '') from dual;
--trim과 같은 역할이지만, 문자열 중간의 공백도 제거할 수 있다.
SELECT replace('good mornimg john', ' ', '') as 공백제거 from dual;

/*
instr() : 문자열에서 특정문자가 위치한 인덱스를 찾아 반환한다.
    형식1] instr(컬럼명, '찾을문자')
        문자열의 처음부터 문자를 찾는다.
    형식2] instrt(컬럼명, '찾을문자', '탐색을 시작할 인덱스', '몇번째 문자');
