...
/
/*
    PL/SQL 문장의 끝에는 반드시 /를 붙여야 하는데, 만약 없다면
    호스트환경으로 빠져나오지 못한다. 즉 PL/SQL 문장을 탈출하기 위해
    필요하다.
...
DECLARE --선언부
    --사원테이블의 특정 컬럼의 타입과 크기를 그래도 참조하는 참조변수 선언
    empNo employees.employee_id%TYPE; --NUMBER(6,0)로 선언
    empSal employees.salary%TYPE;   --NUMBER(8,2)와 같이 선언
    deptId employees.department_id%TYPE; --NUMBER(4,0)와 같이 선언
BEGIN
...
DECLARE
/* 사원 테이블의 전체 컬럼을 참조하는 참조변수로 선언. 테이블명
뒤에 %rowtype을 붙여주면 된다.*/
...
BEGIN
    ...
    SELECT * 
        into emp_row
    /*
    row_row에는 전체컬럼의 정보가 저장되므로 출력시 번수명.컬럼명
    형태로 기술해야한다.*/
...
DECLARE
    ...
    type emp_3type is record(
        ...
        /* 앞에서 정의한 자료형을 통해 복합변수를 선언한다. 이변수는
        3개의 값을 저장할 수 있다.*/
        record emp_3type;
BEGIN
    ...
    from employees where employee_id=100;
    --출력
    dbms_output.put_line(record3.em_id||' '||
                        record3.emp_name||' '||
                        record3.emp_job);
end;
/
...
from employees where employee_id=108;
     ...
     dbms_output.put_line('사원번호 / 사원전체이름 / 급여 / 보너스율');
